...
const PreviewSectionImageText: React.FC<Props> = ({ section, theme }) => {
  return (
    <div
      className={
        clsx(
          "flex justify-evenly max-sm:flex-col items-center",
          { "flex-row-reverse max-sm:flex-col-reverse": ((section as SectionImageText).item.position === 'text_img') }
        )
      }>
      ...
      >
        ...
        >
          <div
            style={{ fontSize: section.item.imgCaptionSize }}
      ...
      >
        ...
        <div className="flex items-between m-4">
          <div
            ...
            className={clsx(
              "w-full outline-none",
              { [modern.textInputBackground]: (theme === "modern")}
            )}
